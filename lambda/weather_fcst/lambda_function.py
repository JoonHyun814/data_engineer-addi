...
def register_partition(glue, dt: str, hr: str, location: str):
    ...
    """
    try:
        table = glue.get_table(DatabaseName="weather", Name="forecast")["Table"]
        sd = dict(table["StorageDescriptor"])
        sd["Location"] = location
        glue.create_partition(
            DatabaseName="weather", TableName="forecast",
...
def lambda_handler(event, context):
    ...
    if records:
        ...
        register_partition(glue, dt_str, hr_str, f"s3://{S3_BUCKET}/prod/weather/forecast/dt={dt_str}/hr={hr_str}/")
