-- 통합 CTV Reach → Mall 방문 전환 집계 v3 (Addi + Abi + ODM)
-- raw_conv_web_v3.sql과 로직은 동일하고, athena-insert-data 공용 상태머신(CreateAthenaQuery Lambda)에서
-- 실행하기 위해 날짜 플레이스홀더만 {date} → {input_date}로 바꾼 버전.
-- s3://ptbwa-athena/query/ptbwa/prod-addi_conv_raw_conv_web_v3.sql 로 업로드해서 사용한다.
-- v2와 차이: 캠페인+IP당 최초 CTV 시청 접점을 찾은 뒤, 그 이후 발생한 모든 GTM 전환 이벤트를
-- 1:N으로 전부 적재한다(raw_conv_Abi.sql / raw_conv_addi.sql / raw_conv_odm.sql의 원본 매칭 방식).
-- v2는 캠페인+mall_ip당 최초 매칭 1건만 남기지만, v3는 같은 IP에서 반복 전환이 발생해도 전부 적재된다.
-- 컬럼도 원본 3개 쿼리 기준으로 log_time / eid / scroll / click / conv_type을 그대로 유지한다.
-- 파티션: year, month, day (Hive-style, 컬럼 순서 마지막에 위치)
INSERT INTO "prod_addi_conv"."raw_conv_web_v3"
