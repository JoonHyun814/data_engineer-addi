-- 초기 적재용 (CTAS) — 이후 일별 적재는 raw_conv_web_home.sql, 구간 적재는 raw_conv_web_home_range.sql 사용
-- raw_conv_web_v2.sql 기반, 적재 테이블만 raw_conv_web_home
-- 실행 전 date_range CTE의 start_yyyymmdd / end_yyyymmdd 수동 설정
-- v1과 달리 stp_id / m_adid / raw_ip / conv_meta / conv_value 컬럼을 추가로 적재한다.

CREATE TABLE "prod_addi_conv"."raw_conv_web_home"
WITH (
    external_location = 's3://ptbwa-da/prod/prod_addi_conv/raw_conv_web_home/',
    format = 'PARQUET',
