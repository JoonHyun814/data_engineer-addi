...
-- raw_conv_web.sql과 로직은 동일하고, athena-insert-data 공용 상태머신(CreateAthenaQuery Lambda)에서
-- 실행하기 위해 날짜 플레이스홀더만 {date} → {input_date}로 바꾼 버전.
-- s3://ptbwa-athena/query/ptbwa/prod-addi_conv_raw_conv_web.sql 로 업로드해서 사용한다.
