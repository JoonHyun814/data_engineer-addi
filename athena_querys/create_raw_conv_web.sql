-- 초기 적재용 (CTAS) — 이후 일별 적재는 raw_conv_all.sql 사용
-- 실행 전 date_range CTE의 start_yyyymmdd / end_yyyymmdd 수동 설정

CREATE TABLE "prod_addi_conv"."raw_conv_web"
WITH (
    external_location = 's3://ptbwa-da/prod/prod_addi_conv/raw_conv_web/',
    format = 'PARQUET',
...
addi_dedup AS (
    SELECT
        ...
        ev,
        ROW_NUMBER() OVER (
            PARTITION BY cmp_no, mall_ip, mall_dt
...
SELECT
    cmp_no, pid, ev, mall_dt, post_datetime, mall_datetime, mall_ip,
    'Addi'                AS cate,
