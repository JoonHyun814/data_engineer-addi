...
-- raw_conv_web_imp.sql과 로직은 동일하고, athena-insert-data 공용 상태머신(CreateAthenaQuery Lambda)에서
-- 실행하기 위해 날짜 플레이스홀더만 {date} → {input_date}로 바꾼 버전.
-- s3://ptbwa-athena/query/ptbwa/prod-addi_conv_raw_conv_web_imp.sql 로 업로드해서 사용한다.
-- {input_date} 플레이스홀더: CreateAthenaQuery Lambda가 처리 날짜로 치환 (YYYY-MM-DD)

INSERT INTO "prod_addi_conv"."raw_conv_web_imp"
...
SELECT
    cmp_no, pid, ev,
    DATE(SUBSTR(post_datetime, 1, 10)) AS imp_dt,
