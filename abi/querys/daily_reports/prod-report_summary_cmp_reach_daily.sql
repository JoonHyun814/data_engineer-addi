-- CREATE TABLE "prod-ptbwa-da"."report_summary_cmp_reach_daily"
--  WITH (
--       format = 'Parquet',
--       external_location = 's3://ptbwa-da/prod/report_summary_cmp_reach_daily/',
--       partitioned_by = ARRAY['year', 'month', 'day']
--   ) AS
 
INSERT INTO "prod-ptbwa-da"."report_summary_cmp_reach_daily"
...
with acp as (
    ...
    SELECT
        ...
        END AS SUM_REACH3
    FROM
        "prod-ptbwa-da"."report_summary_cmp_reach_daily" as summary
    INNER JOIN
        acp
    on
        summary.cmpno=acp.cmpno
    WHERE
...
reach_postback AS (
    ...
    inner join
        acp
    on
        acp.cmpno=ab_postback_log.cmp_no
    CROSS JOIN
        ag_min
    WHERE
        CONCAT(year, '-', month, '-', day) between cast(date_format(CAST(ag_min.min_startdt AS timestamp), '%Y-%m-%d') as varchar) and '{input_date}'
    ...
    FROM (
        SELECT
            ...
            app_id
        FROM (
			select cmp_no, log_type, ifa, media_id, app_id from "prod-ptbwa-dw".postback_log WHERE year='{input_year}' AND month='{input_month}' AND day='{input_day}' and media_id = 'CX3NWBJED7HA'
			union all
			select cmp_no, log_type, ifa, media_id, app_id from "prod-ptbwa-dw".ab_postback_log WHERE year='{input_year}' AND month='{input_month}' AND day='{input_day}' and ctv_media is not null and ctv_media!=''
		 )
    ...
    SELECT
        ...
        cmp_no,
        ifa,
        ROW_NUMBER() OVER (PARTITION BY ifa, cmp_no ORDER BY day) AS view_rank
    FROM 
        reach_postback
    WHERE
        log_type = 'v_complete'
    ...
    SELECT
        cmp_no,
        SUM(view_rank_1) OVER (PARTITION BY cmp_no) AS cumulative_reach_1_plus,
        SUM(view_rank_2) OVER (PARTITION BY cmp_no) AS cumulative_reach_2_plus,
        SUM(view_rank_3) OVER (PARTITION BY cmp_no) AS cumulative_reach_3_plus
    FROM
...
LEFT JOIN
    reach_sum
ON
    reach_sum.cmpno=d.cmp_no
    and reach_sum.cmpno=fcr.cmp_no
ORDER BY 
    1, 2, 3, 4, 5