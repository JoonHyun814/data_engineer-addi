...
with acp as (
    ...
    where
        '2026-04-01' between cast(date_format(CAST(startdt AS timestamp), '%Y-%m-%d') as varchar) and cast(date_format(CAST(enddt AS timestamp), '%Y-%m-%d') as varchar)
        and logparsefg='Y'
...
reach_postback AS (
    ...
    CROSS JOIN
        ag_min
    WHERE
        CONCAT(year, '-', month, '-', day) between cast(date_format(CAST(ag_min.min_startdt AS timestamp), '%Y-%m-%d') as varchar) and '2026-04-01'
    ...
    FROM (
        SELECT
            ...
            app_id
        FROM (
			select cmp_no, log_type, ifa, media_id, app_id from "prod-ptbwa-dw".postback_log WHERE year='2026' AND month='04' AND day='01' and media_id = 'CX3NWBJED7HA'
			union all
			select cmp_no, log_type, ifa, media_id, app_id from "prod-ptbwa-dw".ab_postback_log WHERE year='2026' AND month='04' AND day='01' and ctv_media is not null and ctv_media!=''
		 )
